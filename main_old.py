...
def flip_spin(x):
    ...
    if x == 1:
        ...
        raise Exception("Input has to be +1 or -1.")
...
class Lattice:
    ...
    def modify_system_one_spin(self):
        ...
        index = self.rng.integers(low=0, high=self.n_sites, size=2)
    ...
    def generate_spins(self, start_temp="inf"):
        ...
        if start_temp == "inf":
            ...
            self.lattice = np.ones([self.n_sites, self.n_sites])
        else:
    ...
    def get_energy_single_spin(self, lattice, index):
        ...
        for i in range(0, 2):
            for j in range(0, 2):
                ...
                l = 2 * j - 1
                energy[i + j] = (
                    lattice[index[0], index[1]]
                    * lattice[(index[0] + k) % N, (index[1] + l) % N]
                )
    ...
    def calc_delta_energy(self, lattice_old, lattice_new, index):
        ...
        delta = self.get_energy_single_spin(
            lattice_new, index
        ) - self.get_energy_single_spin(lattice_old, index)
        return delta
    ...
    def get_energy(self, lattice):
        ...
        lattice_product = np.zeros([4, self.n_sites, self.n_sites])
        k = 0
        for i in range(0, 2):
            for j in range(0, 2):
                # Shift array
...
class Simulation:
    ...
    def run_simulation_one_step(self):
        ...
        randfloat = self.simulation_rng.random()
        # print(f"delta energy = {delta_energy}, chance = {np.exp((-1/self.system.T) * delta_energy)}, float ={randfloat} ")
        if (
            delta_energy < 0
            or np.exp((-1 / self.system.temperature) * delta_energy)
            > 1 - randfloat  # this is not currect, k_b is missing
        ):
    ...
    def run_simulation_one_step_one_spin(self):
        index = self.system.modify_system_one_spin()
        delta_energy = self.system.calc_delta_energy(
            self.system.lattice, self.system.new_lattice, index
        )
        self.system.new_energy = self.system.energy + delta_energy
        randfloat = np.random.default_rng().random()
        if (
            delta_energy < 0
            or np.exp((-1 / self.system.temperature) * delta_energy) > randfloat
        ):
            # TODO verify if correct
            self.system.lattice = self.system.new_lattice
            self.system.energy = self.system.new_energy
